...
def edit_channel(id):
    ...
    if len(new_num) > 0 and int(new_num) > 0:
        ...
        if int(new_num) in channels_nums:
            xbmcgui.Dialog().notification('Rebit.tv', 'Číslo kanálu ' + new_num +  addon.getLocalizedString(300212) + channels_nums[int(new_num)]['name'], xbmcgui.NOTIFICATION_ERROR, 5000)
        else:  
...
def change_channels_numbers(from_number, direction):
    ...
    if len(change) > 0:
        ...
        if change > 0:
            ...
            xbmcgui.Dialog().notification('Rebit.tv', addon.getLocalizedString(300213), xbmcgui.NOTIFICATION_ERROR, 5000)
    else:  
        xbmcgui.Dialog().notification('Rebit.tv', addon.getLocalizedString(300213), xbmcgui.NOTIFICATION_ERROR, 5000)
...
def list_channels_list_backups(label):
    ...
    if len(backups) > 0:
        ...
        xbmcgui.Dialog().notification('Rebit.tv', addon.getLocalizedString(300214), xbmcgui.NOTIFICATION_INFO, 5000)          
...
def add_channel_group(label):
    ...
    group = input.getText()
    if len(group) == 0:
        xbmcgui.Dialog().notification('Rebit.tv', addon.getLocalizedString(300216), xbmcgui.NOTIFICATION_ERROR, 5000)
        sys.exit()          
    channels_groups = Channels_groups()
    if group in channels_groups.groups:
        xbmcgui.Dialog().notification('Rebit.tv', addon.getLocalizedString(300217), xbmcgui.NOTIFICATION_ERROR, 5000)
        sys.exit()          
...
def select_channel_group(group):
    ...
    if (not group in channels_groups.channels or len(channels_groups.channels[group]) == 0) and group != 'all':
        xbmcgui.Dialog().notification('Rebit.tv', 'Vybraná skupina je prázdná', xbmcgui.NOTIFICATION_WARNING, 5000)    
...
class Channels:
    ...
    def get_channels(self):
        ...
        session = Session()
        api = API()
        response = api.call_api(url = 'https://bbxnet.api.iptv.rebit.sk/television/channels', data = None, method = 'GET', headers = api.get_headers(token = session.access_token, device_id = session.device_id))
        if 'data' not in response:
            xbmcgui.Dialog().notification('Rebit.tv',addon.getLocalizedString(300211), xbmcgui.NOTIFICATION_ERROR, 5000)
            sys.exit() 
        for channel in response['data']:
            if 'icon' in channel:
                ...
                image = None
            channels.update({channel['id'] : {'channel_number' : int(channel['channel']), 'rebit_number' : int(channel['channel']), 'name' : channel['title'], 'id' : channel['id'], 'logo' : image, 'visible' : True}})
        return channels
    ...
    def reset_channels(self):
        ...
        self.valid_to = -1
        self.load_channels()
        xbmcgui.Dialog().notification('Rebit.tv', addon.getLocalizedString(300318), xbmcgui.NOTIFICATION_INFO, 5000)
                    ...
                    if 'valid_to' in data:
                        ...
                        try:
                            with codecs.open(filename, 'w', encoding='utf-8') as file:
                                file.write('%s\n' % data)
                                xbmcgui.Dialog().notification('Rebit.tv', addon.getLocalizedString(300320), xbmcgui.NOTIFICATION_INFO, 5000) 
                        except IOError:
                            xbmcgui.Dialog().notification('Rebit.tv', addon.getLocalizedString(300321), xbmcgui.NOTIFICATION_ERROR, 5000)      
                except:
                    xbmcgui.Dialog().notification('Rebit.tv', addon.getLocalizedString(300319), xbmcgui.NOTIFICATION_ERROR, 5000)
        else:
            xbmcgui.Dialog().notification('Rebit.tv', addon.getLocalizedString(300322), xbmcgui.NOTIFICATION_ERROR, 5000)      

    def merge_channels(self):
        rebit_channels = self.get_channels()
        max_number = 0
        if len(self.channels) > 0:
            max_number = self.channels[max(self.channels, key = lambda channel: self.channels[channel]['channel_number'])]['channel_number']
        for channel in sorted(rebit_channels, key = lambda channel: rebit_channels[channel]['channel_number']):
            if channel in self.channels:
                if self.channels[channel]['name'] != rebit_channels[channel]['name']:
                    self.channels[channel].update({'name' : rebit_channels[channel]['name']})
                if self.channels[channel]['rebit_number'] != rebit_channels[channel]['rebit_number']:
                    self.channels[channel].update({'rebit_number' : rebit_channels[channel]['rebit_number']})
                if self.channels[channel]['logo'] != rebit_channels[channel]['logo']:
                    self.channels[channel].update({'logo' : rebit_channels[channel]['logo']})
            else:
                max_number = max_number + 1
                rebit_channels[channel]['channel_number'] = max_number
                self.channels.update({channel : rebit_channels[channel]})
        for channel in list(self.channels):
            if channel not in rebit_channels:
    ...
    def save_channels_groups(self):
        ...
        if(len(self.groups)) > 0:
            try:
                ...
                xbmcgui.Dialog().notification('Rebit.tv', 'Chyba uložení skupiny', xbmcgui.NOTIFICATION_ERROR, 5000)      
        else:
            if os.path.exists(filename):
                os.remove(filename) 
